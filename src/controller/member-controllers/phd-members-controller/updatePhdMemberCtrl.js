...
const updatePhdMemberCtrl = async (req, res) => {
  ...
  try {
    ...
    if (req.file) {
      const newMemberImageUpload = await cloudinaryConfig.uploader.upload(
        filePath,
        {
      ...
      newMemberImage = newMemberImageUpload.secure_url;
      newCloudPublicId = newMemberImageUpload.public_id;
